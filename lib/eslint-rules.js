...
function define(category, minEcmaVersion, ruleConfig)
{
    const match = category.match(/^plugin:(.*)/);
    if (match)
        ruleConfig = prependToPropertyNames(ruleConfig, `${match[1]}/`);
    const definition = { category, minEcmaVersion, ruleConfig };
    return definition;
}
...
const NO_UNUSED_VARS_CONFIG = ['error', { vars: 'local' }];
