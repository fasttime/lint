...
function createBaseRules(language, ecmaVersion, configRules = { })
{
    const ruleConfigs = [];
    ...
    if (language === 'ts')
    {
        ...
        for (const [key, value] of Object.entries(configRules))
        {
            if (!key.includes('/') && tsRules.hasOwnProperty(key))
            {
...
function createESLint(config, language)
{
    ...
    if (ecmaVersion === undefined)
    {
        ...
        ecmaVersion -= 2009;
    const { defaultDialectName, envs, fix, rules: configRules, src, ...overrideConfig } = config;
    const env = createBaseEnv(envs, ecmaVersion);
    const plugins = ['@fasttime', 'node'];
    const rules = createBaseRules(language, ecmaVersion, configRules);
    if (language === 'ts')
    {
